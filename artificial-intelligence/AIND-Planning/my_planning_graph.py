...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # See if all the prerequisite literals for the action hold in the S literal level
            if action_node.prenodes.issubset(s_level):
                # Add this node to current A action level
                action_nodes.add(action_node)

                # Connect the node to the previous S literal level
                for prenode in action_node.prenodes:
                    for state_node in s_level:
                        if prenode.literal == state_node.literal:
    ...
    def add_literal_level(self, level):
        ...
        for action_node in previous_a_level:
            ...
            # Process literal nodes produced by action nodes
            for literal_node in action_node.effnodes:
                # Add this node to current S literal level
                literal_nodes.add(literal_node)

                # Connect the node to the previous action level
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent1 in node_s1.parents:
            for parent2 in node_s2.parents:
                if parent1.is_mutex(parent2):
                    if len(node_s1.parents.intersection(node_s2.parents)) == 0:
                        return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        def find_first_level(goal):
            # Check all levels and nodes for the goal until first instance found
        ...
        # For each goal in the problem, determine the level cost and add it in
        level_sum = 0
        for goal in self.problem.goal:
            level_sum += find_first_level(goal)
